...
export var isRune = function (kindOrString) {
    if (typeof kindOrString === "number") {
        return RuneByKind[kindOrString];
    }
    else {
        return kindOrString === null || kindOrString === void 0 ? void 0 : kindOrString.startsWith("rune");
    }
};
export var RUNE = {
    SAT: 161,
...
export var RuneByKind = Object.entries(RUNE).reduce(function (acc, _a) {
    var name = _a[0], kind = _a[1];
    acc[kind] = name.toLowerCase();
    return acc;
}, {});
export var RuneList = Object.keys(RUNE).map(function (rune) { return rune.toLowerCase(); });
export var getRuneNameFromItem = function (rankOrString) {
    var _a;
    var rune;
    if (typeof rankOrString === "string") {
        _a = rankOrString.split(":")[0].split("-"), rune = _a[1];
    }
...
export var getRuneFromItem = function (rankOrString) {
    var rune = getRuneNameFromItem(rankOrString);
    return runeKind[rune];
};
export var getJewelRequirement = function (bonus) {
    var requirement = 4;
    bonus = toArray(bonus);
    if (bonus.length >= 5) {
        requirement = 60;
...
export var getJewelSkinIndex = function (level) {
    var index = "";
    if (level === 3) {
        index = "1";
    }
...
export var getHighestSocketRequirement = function (rawSocket) {
    var requirement = 1;
    for (var i = 0; i < rawSocket.length; i++) {
        if (!rawSocket[i])
            continue;
        var rawRequirement = 1;
        if (typeof rawSocket[i] === "number") {
            (rawRequirement = runeKind[RuneList[rawSocket[i] - 1]].requirement);
        }
        else if (typeof rawSocket[i] === "string") {
            var _a = rawSocket[i].split("|"), rawBonus = _a[2];
...
export var getRune = function (rawRune) {
    if (!rawRune || !RuneList.includes(getRuneNameFromItem(rawRune)))
        return;
    var rune = getRuneFromItem(rawRune);
    return rune;
};
export var getRunesBonus = function (runes) {
    var attributes = {};
    _.forEach(runes, function (kind) {
        if (!kind || typeof kind !== "number")
            return;
        var rune = getRuneFromItem(kind);
        Object.entries(rune.attribute).map(function (_a) {
...
export var getRunewordBonus = function (_a) {
    var _b;
    var _c;
    var isUnique = _a.isUnique, socket = _a.socket, type = _a.type;
    var runeword;
    var runewordBonus;
    var wordSocket;
    if (!isUnique && (socket === null || socket === void 0 ? void 0 : socket.length) && !socket.some(function (s) { return s === 0 || typeof s !== "number"; })) {
        wordSocket = socket.map(function (s) { return RuneList[s - 1]; }).join("-");
        (_b = ((_c = Runewords[type]) === null || _c === void 0 ? void 0 : _c[wordSocket]) || {}, runeword = _b.name, runewordBonus = _b.bonus);
    }
    return { runeword: runeword, runewordBonus: runewordBonus, wordSocket: runeword ? wordSocket : null };
};
export var Runewords = {
    weapon: {
        "al-sat-mir-nan": {
