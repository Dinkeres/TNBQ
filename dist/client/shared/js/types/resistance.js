...
export var PLAYER_MAX_RESISTANCES = 90;
export var PLAYER_MAX_ATTACK_SPEED = 50;
export var DEFAULT_ATTACK_SPEED = 800;
export var DEFAULT_ATTACK_ANIMATION_SPEED = 50;
export var PLAYER_MAX_EXTRA_GOLD = 75;
export var PLAYER_MAX_MAGIC_FIND = 75;
export var mobEnchant = {
...
export var resistanceToDisplayMap = {
...
export var enchantToDisplayMap = {
...
var resistanceToLowerResistanceMap = {
...
export var elements = ["magic", "flame", "lightning", "cold", "poison", "spectral"];
export var getRandomElement = function () { return _.shuffle(elements)[0]; };
export var calculateResistance = function (resistance, curseResistances) {
    if (curseResistances === void 0) { curseResistances = 0; }
    var calculatedResistances = resistance - curseResistances > PLAYER_MAX_RESISTANCES ? PLAYER_MAX_RESISTANCES : resistance - curseResistances;
    if (calculatedResistances < 0) {
        calculatedResistances = 0;
    }
    return calculatedResistances;
};
export var calculateAttackSpeedCap = function (rawAttackSpeed, weaponKind) {
    var attackSpeedByWeight = 0;
    var weight;
    if (weaponKind) {
        weight = getWeaponWeightbyKind(weaponKind);
        attackSpeedByWeight = getAttackSpeedBonusFromStringMap[weight];
    }
    var attackSpeed = rawAttackSpeed + attackSpeedByWeight;
    return attackSpeed > PLAYER_MAX_ATTACK_SPEED ? PLAYER_MAX_ATTACK_SPEED : attackSpeed;
};
export var calculateExtraGoldCap = function (extraGold) {
    return extraGold > PLAYER_MAX_EXTRA_GOLD ? PLAYER_MAX_EXTRA_GOLD : extraGold;
};
export var calculateMagicFindCap = function (magicFind) {
    return magicFind > PLAYER_MAX_MAGIC_FIND ? PLAYER_MAX_MAGIC_FIND : magicFind;
};
export var getResistance = function (mob, attacker) {
    var resistances = __assign({}, DefaultResistances);
    if (mob.type === "mob") {
...
export var calculateLowerResistances = function (resistances, bonus) {
    return Object.keys(resistances).reduce(function (acc, resistance) {
        acc[resistance] =
