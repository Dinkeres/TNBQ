  ...
  constructor(connection, worldServer, databaseHandler) {
    ...
    this.connection.listen(async rawMessage => {
      ...
      if (action === Types.Messages.CREATE || action === Types.Messages.LOGIN) {
        ...
        if (!password) {
          // Password is required for new character creation
          if (action === Types.Messages.CREATE) {
            self.connection.sendUTF8("passwordcreate");
            self.connection.close("Password is required for new character");
            return;
          }
          // For login, check if password is required
