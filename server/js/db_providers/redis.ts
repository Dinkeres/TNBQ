...
class DatabaseHandler {
  ...
  async createPlayer(player) {
    ...
    let stash = JSON.stringify(new Array(STASH_SLOT_COUNT).fill(0));
    let gems = JSON.stringify(new Array(GEM_COUNT).fill(0));
    let artifact = JSON.stringify(new Array(ARTIFACT_COUNT).fill(0));
    let upgrade = JSON.stringify(new Array(UPGRADE_SLOT_COUNT).fill(0));
    let trade = JSON.stringify(new Array(TRADE_SLOT_COUNT).fill(0));
    let waypoints = JSON.stringify([1, 0, 0, 2, 2, 2, 2, 2, 2, 2]);
    let settings = JSON.stringify(defaultSettings);
    ...
    await this.client
      ...
      .hSet(userKey, "pet", "")
      .hSet(userKey, "shield", "")
      .hSet(userKey, "settings", settings)
      .hSet(userKey, "ring1", "")
      .hSet(userKey, "ring2", "")
      .hSet(userKey, "amulet", "")
      .hSet(userKey, "nanoPotions", 0)
      .hSet(userKey, "gems", gems)
      .hSet(userKey, "artifact", artifact)
      .hSet(userKey, "upgrade", upgrade)
      .hSet(userKey, "trade", trade)
      .hSet(userKey, "expansion1", 0)
      .hSet(userKey, "expansion2", 0)
      .hSet(userKey, "waypoints", waypoints)
  ...
  async upgradeItem(player: Player) {
    ...
    // SECURITY FIX: Reject invalid upgrade attempts (prevents dupe exploits)
    // If there's an item but no valid upgrade material (scroll/stone/rune), reject the attempt
    if (filteredUpgrade.length > 0) {
      const hasValidUpgradeMaterial = filteredUpgrade.some(item => {
        if (!item) return false;
        return (
          item.startsWith("scroll") ||
          item.startsWith("stone") ||
          item.startsWith("rune") ||
          item.startsWith("chest") ||
          item.startsWith("petegg") ||
          item.startsWith("expansion")
        );
      });

      if (!hasValidUpgradeMaterial) {
        // Invalid attempt - just return without processing, items stay in panel
        console.info(`Invalid upgrade attempt for player ${player.name} - no valid upgrade material`);
        // Don't save anything, just return - items stay in panel unchanged
        return;
      }
    }
  ...
  async passwordIsRequired(player) {
    ...
    try {
      ...
      // If password exists in database, require it for login
      // If no password exists, new players can create one without password
      if (password) {
